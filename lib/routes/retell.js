...
const service = ({logger, makeService}) => {
  ...
  svc.on('session:new', async(session) => {
    ...
    session
      ...
      .on('transfer-status', onTransferStatus.bind(null, session))
...
const onClose = (session, code, reason) => {
  ...
  if (session.parent_call_sid && (session.parent_call_sid in sessions)) {
...
const onTransferStatus = (session, evt) => {
  const {logger} = session.locals;
  logger.info('transfer-status', session, evt)  
}
